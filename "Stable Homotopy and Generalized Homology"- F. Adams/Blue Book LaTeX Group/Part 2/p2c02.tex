...
\begin{notes}
It is justifiable to take $x^K$ in ${\breve K}^0(\mathbb {CP}^\infty)$ instead of ${\tilde K}^2(\mathbb {CP}^\infty)$, because it makes the ``$n$-th Chern class in $K$-cohomology'' lie in dimension 0 instead of dimension $2n$, so that it is more conveniently related to bundles and representation-theory. Also we get a better formula at \plscite{(2.9)} below. The unit $u^K$ is the usual generator in $\pi_2(K)$; this provides some justification for writing $i^\ast x = u \gamma$ rather than $\gamma = u i^\ast x$.
\end{notes}
\end{examples}
...
\end{enumerate}
\end{lemma}
%TODO: I am not 100% sure where this proof should end
\begin{proof}
...
The proof of part (i) is easy, by considering the pairing of these spectral sequences with those of \plscite{(2.5)(i)}. (Compare \cite[p.~21]{adams3}, where however one is arguing in the opposite direction) This leads immediately to parts (ii) and (iii). We see that in part (i), the $E^2$-term of each spectral sequence has a $\pi_\ast(E)$-base consisting of the appropriate elements $\beta_i$ or $\beta_{ij}$. This leads to parts (iv) and (v).
