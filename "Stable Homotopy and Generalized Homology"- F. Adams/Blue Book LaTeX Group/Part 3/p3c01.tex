...
Now of course to solve stable problems, or to compute groups such as $[X, Y]$ or $\pi_{n + r}(\mathrm{MSO}(n))$, we need computable invariants. In the first instance this means homology and cohomology, but we could certainly agree to go as far as generalized homology and cohomology theories. I will suppose it is known that a generalized homology or cohomology theory is a functor $K_*$ or $K^*$ that satisfies the first six axioms of \plscite{Eilenberg-Steenrod [6]}, but not necessarily the seventh, the dimension axiom. I suppose it is known that the material of Eilenberg-Steenrod Chapter 1 carries over to this situation. For example, if $X$ is a space with base-point one can define reduced groups $\tilde K_*(X)$, $\tilde K^*(X)$; and one can define a suspension isomorphism
\begin{align*}
