...
The notion of a spectrum is due to Lima \plscite{[7]}. It is generally supposed that G. W. Whitehead also had something to do with it, but the latter takes a modest attitude about that.
...
\[
\varepsilon_n: SE_n \lar{} E_{n+1}
\]
or
\[
\varepsilon_n': E_n \lar{} \Omega E_{n+1}
\]
Of course giving a map $\varepsilon_n$ is equivalent to giving a map $\varepsilon_n'$, as $S$ and $\Omega$ are adjoint. There is one other variant; if we choose to work with connected spaces, then $E_n$ will automatically map into $\Omega_0 E_{n+1}$, where $\Omega_0$ is the component of the base-point in $\Omega$; we might prefer to write
\[
...
\[
i_\alpha^\ast:
\widetilde{K^n} \left( \bigvee_\alpha X_\alpha \right) \lar{} K^n(X_\alpha).
\]
Let
\[
\theta: 
\widetilde{K^n} \left( \bigvee_\alpha X_\alpha \right) \lar{} \prod_{\alpha\in A} \widetilde{K^n}(X_\alpha)
\]
be the homomorphism with components $i_\alpha^\ast$. We assume that $\theta$ is an isomorphism (for all choices of $\{X_\alpha\}$ and $n$.)

We can now apply the representability theorem of E. H. Brown \plscite{[4]}. We see that there exist connected CW-complexes $E_n$ with base-point and natural equivalences
\[
...
\[
\sigma: \widetilde{K^n} \lar{\cong} \widetilde{K^{n+1}}(SX).
\]
Here $SX$ is considered as the union of two cones $CX$ and $C'X$ over the same copy of $X$. The suspension isomorphism is defined as
\[
\begin{tikzcd}
...
\end{tikzcd}
\]
The map $\delta$ is the coboundary for the exact sequence of the triple $(CX,X,\mathrm{pt.})$. The vertical isomorphism is also induced by the collapsing map $(CX,X)\lar{} (SX,\mathrm{pt.})$.
...
Similarly, we can take $K^\ast$ to be real K-theory. The corresponding spectrum is called the $\te{BO}$-spectrum. Every eighth term $E_{8n}$ is the space $BO$, or $\mathbb Z \times \te{BO}$, depending on whether you choose to work with connected spaces or not. Each term $E_{8n+4}$ is the space $\te{BSp}$.
\end{example}
...
\end{cases}
$
 with the obvious maps. We might define a spectrum $F$ to be a \emph{suspension spectrum}\index{suspension spectrum (S-spectrum)} or \emph{S-spectrum} if
\[
\varphi_n: SF_n \lar{} F_{n+1}
\]
is a weak homotopy equivalence for $n$ sufficiently large. Then this spectrum $E$ would be an S-spectrum, but usually not an $\Omega$-spectrum. $E$ is called the \emph{suspension spectrum} of $X$. In particular, the \emph{sphere spectrum}\index{suspension spectrum (S-spectrum)!sphere spectrum} $S$ is the suspension spectrum of $S^0$; it has $n^{\text{th}}$ term $S^n$ for $n\geq 0$.
\end{example}
...
Similar remarks apply to the Thom spectra $\te{MSO}$, $\te{MSpin}$, $\te{MU}$, $\te{MSU}$ and $\te{MSp}$. However, $\te{MU}(n)$ is the $2n\nth$ term of the spectrum $\te{MU}$, the $(2n+1)\nth$ term being $\te{MU}\wedge S^1$ (because in the complex case we have $\te{M}(1)=S^2$.) Similarly for $\te{MSU}$. For $\te{MSp}$, the term $E_{4n+\varepsilon}$ is $\te{MSp}(n)\wedge S^\varepsilon$ for $\varepsilon=0,1,2,3$.
\end{example}
...
If $E$ is and $\Omega$-spectrum or an $\Omega_0$-spectrum, then the homomorphism
\[
\pi_{n+r}(E_n)
\lar{}
\pi_{n+r+1}(E_{n+1})
...
so that
\[
\pi_r(E) = \lim_{n\to \infty} \pi_{n+r}(S^nX).
\]
...
Similarly, of course, we can define relative homotopy groups. To do so we need subobjects. Let $X$ be a spectrum; then a \emph{subspectrum}\index{subspectrum} $A$ of $X$ consists of subspaces $A_n\subseteq X_n$ such that the structure map $\xi_n: SX_n \lar{} X_{n+1}$ maps $SA_n$ into $A_{n+1}$. Of course we take $\left. \xi_n \right\rvert SA_n$ as the structure map $\alpha_n$ for $A$. And if we think in terms of maps $\xi_n': X_n\to\Omega X_{n+1}$, we ask that $\xi_n'$ maps $A_n$ into $\Omega A_{n+1}$.
...
S^{m-1} \wedge S^1.
\]
So at this point we prefer to interpret $SX_n$ as $X_n\wedge S^1$, as is done in Puppe's paper on stable homotopy theory. With this convention, the ladder diagram commutes; we can define
\[
\pi_r(X,A)
=
...
We have seen how to associate a spectrum to a generalized cohomology theory. The converse is also possible: with any spectrum $E$ we can associate a generalized homology theory and a generalized cohomology theory. This is due to G. W. Whitehead, in a celebrated paper \plscite{[17]}. I'll get back to this later. If we have a spectrum $E$, it is very convenient to write $E_\ast$ and $E^\ast$ for this associated homology and cohomology theories. I will also reverse this. Ordinary homology and cohomology (with $\mathbb Z$ coefficients) are always written $H_\ast$, $H^\ast$; therefore, $H$ will mean the Eilenberg-MacLane spectrum for the group $\mathbb Z$. (For coefficients in the group $G$, we write $HG$.) This frees the letter $K$ for other uses. Classical complex K-theory is always written $K^\ast$; therefore, $K$ will mean the BU-spectrum. This is fine, because I would anyway need notation to distinguish the space $\te{BU}$ from the BU-spectrum. Similarly, we write $\te{KO}$ for the BO-spectrum.
...
I say $E$ is a \emph{CW-spectrum}\index{CW-spectrum} if
    ...
    \item[(ii)] each map $\varepsilon_n: SE_n \lar{} E_{n+1}$ is an isomorphism from $SE_n$ to a sub-complex of $E_{n+1}$.
    \end{enumerate}
...
\begin{notes}
    ...
    \item[(ii)]  An isomorphism between CW-complexes is a homeomorphism $h$ such that $h$ and $h^{-1}$ are cellular. The CW structure on $SE_n$ is the obvious one on $E_n\wedge S^1$, where $S^1$ is regarded as a CW-complex with one 0-cell and one 1-cell. Thus $SE_n$ has a base-point and one cell $Sc_\alpha$ for each cell $c_\alpha$ of $E_n$ other than the base-point.
    \item[(iii)] It would be possible to identify $SE_n$ with its image under $\varepsilon_n$ and so suppose $SE_n\subset E_{n+1}$. Sometimes it may be convenient to speak in this way. On the whole, it seems best to leave the definition as I've given it.
    \end{enumerate}
\end{notes}
...
This amounts to saying that if you have a cell $c$ in $E_n$, a map need not be defined on it at once; you can wait till $E_{m+n}$ before defining the map on $S^mc$. The slogan is, ``cells now -- maps later.''
...
\begin{notes}
    ...
    \item[(ii)] It would amount to the same to say that two functions $f': E'\lar{} F$, $f'': E''\lar{} F$ are equivalent if their restrictions to $E'\cap E''$ coincide. This comes from the following fact: if $g,h: K\lar{} L$ are maps of CW-complexes with base-point, and $Sg=Sh$, then $g=h$.
    \end{enumerate}
\end{notes}
...
In order to validate our category we give one small result. Let $K$ be a finite CW-complex, and let $E$ be its suspension spectrum, so that $E_n=S^nK$ for $n\geq 0$. Let $F$ be any spectrum.

\begin{proposition}
We have
\[
[E,F]_r = \lim_{n\to\infty} [S^{n+r}K,F_n].
\]
In particular,
\[
[S,F]_r = \pi_r(F).
...
\end{proposition}
\begin{proof}
For any map $f: S^{n+r}K\lar{} F_n$ we can define a corresponding map between spectra by taking its component on $E_{n+r}$ to be $f: S^{n+r}K\lar{}F_n$; the higher components are then forced. In fact, they are
\[
S^{m+n+r}K
\lar{S^mf}
S^mf_n
...
F_{m+n}.
\]
Suppose two maps $f: S^{n+r}K\lar{}F_n$, $g:S^{m+r}K\lar{}F_n$ give the same element of the direct limit. Then for some $p$, the maps
    \begin{gather*}
    S^{p+r}K
    \lar{S^{p-n}f}
    S^{p-n}F_n
